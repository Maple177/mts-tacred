...
def distance_loss(output,target,masks):
    ...
    for sent_embs, gold_matrix, mask in zip(output,target,masks):
        ...
        #np.save(open("sent_embs.npy","wb"),sent_embs.detach().cpu().numpy())
        #np.save(open("gold_mats.npy","wb"),gold_matrix.detach().cpu().numpy())
        #np.save(open("mask.npy","wb"),mask.detach().cpu().numpy())
        pred_matrix = ((sent_embs.unsqueeze(1)-sent_embs.unsqueeze(0))**2).sum(-1)
        #np.save(open("pred_mats.npy","wb"),pred_matrix.detach().cpu().numpy())
        ...
        #np.save(open("diff.npy","wb"),torch.abs(pred_matrix-gold_matrix**2).detach().cpu().numpy())
        #np.save(open("masked_diff.npy","wb"),torch.abs(pred_matrix - gold_matrix**2).masked_fill_(mask.unsqueeze(0),0).masked_fill_(mask.unsqueeze(1),0).detach().cpu().numpy())
        #assert 1 == 0, "exit"
